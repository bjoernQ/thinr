...
public class CancelFlow2Test extends TestBase {

    private static String staticResult;

    @Test
    public void testNoCancel() throws Exception {
        Thinr.task(mock(Context.class), "name", CancelFlow2Test.class, String.class)
                .endsInBackground((appCtx, param, flowControl) -> {
                    CancelFlow2Test.staticResult = "cancel" + flowControl.isCancelled();
                })
        ...
        doNextAsyncTask(asyncTasks);

        assertEquals(0, asyncTasks.size());
        assertEquals(0, runnables.size());

        assertEquals("cancelfalse", staticResult);

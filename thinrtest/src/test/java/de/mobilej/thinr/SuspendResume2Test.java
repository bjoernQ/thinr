...
public class SuspendResume2Test extends TestBase {

    @Test
    public void testLambda() throws Exception {

        Thinr.task(mock(Context.class), "name", SuspendResume2Test.class, String.class)
                .inBackground(String.class, (appCtx, param, flowCtrl) -> "#1(" + param + ")")
                .onMain((target, param1) -> {
                    target.result = "#2(" + param1 + target.field + ")";
                    return "#2(" + param1 + target.field + ")";
                })
                .inBackground((appCtx2, param2, flowCtrl) -> "#3(" + param2 + ")")
